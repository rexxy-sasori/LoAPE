...
def __main(core, cpu_load, duration, plot, sampling_interval,frequency):
    ...
    if frequency==-1:
        ...
        for c in core:
            res = os.popen(f"cpupower -c {c} frequency-set -g conservative").readlines()
            print(f"core {c} freq: {get_core_freq(c)}, p-state: conservative")
    elif frequency==1:
       for c in core:
            res = os.popen(f"cpupower -c {c} frequency-set -g powersave").readlines()
            print(f"core {c} freq: {get_core_freq(c)}, p-state: powersave")
    elif frequency==2:
        for c in core:
            res = os.popen(f"cpupower -c {c} frequency-set -g ondemand").readlines()
            print(f"core {c} freq: {get_core_freq(c)}, p-state: ondemand")
    elif frequency==3:
        for c in core:
            res = os.popen(f"cpupower -c {c} frequency-set -g performance").readlines()
