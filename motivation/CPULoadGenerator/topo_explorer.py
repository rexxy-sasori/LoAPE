...
def topo_explorer(util, core_num):
    duration = 2 * 60
    
    coreids = np.arange(core_num)

    # selected_coreids = np.random.choice(coreids, core_num)

    core_args = ""
    for core_id in coreids:
        core_args += f"-c {int(core_id)} "
    
    util_args = f"-l {util}"
    cmd = f"{script_path} -f 0 {util_args} {core_args} -d {duration}"
    print(cmd)
    exec_cmd(cmd, EXECUTE_CMD)
...
if __name__ == "__main__":
    cpu_core_ranges = np.linspace(20, CPU_COUNT, 5).astype(np.int32)
    utilization_ranges = np.linspace(0.1, 0.8, 5)

    all_options = np.array(list(itertools.product(cpu_core_ranges, utilization_ranges)))

    for core_num, util in all_options:
        topo_explorer(util, int(core_num))
